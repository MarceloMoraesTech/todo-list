...
export const getAllTasks = (request: any, response: any) => {
  pool.query('SELECT * FROM task', (error, results: any) => {
    if (error) {
      throw error;
    }
...
export const getTaskById = (request: any, response: any) => {
  pool.query(
    `select * from task where id = '${request.params.id}'`,
    (error, results) => {
      if (error) {
...
export const postTask = (request: any, response: any) => {
  pool.query(
    `INSERT INTO TASK (description, duedate, status, list)
    ...
    (error, results) => {
      ...
      response.status(201); //201 tem que end no fim
      response.end();
    }
  );
};
...
export const updateTask = (request: any, response: any) => {
  pool.query(
    `update task set description = '${request.body.description}', duedate = '${request.body.duedate}', status = '${request.body.status}', list = '${request.body.list}' where id = '${request.params.id}'`,
    (error, results) => {
      if (error) {
        response.status(404).send(`O id: ${request.params.id} não existe`);
      } else {
...
export const deleteTask = (request: any, response: any) => {
  pool.query(
    `delete from task where id = '${request.params.id}'`,
    (error, results) => {
      if (error) {
        response.status(404).send(`O id: ${request.params.id} não existe`);
      } else {
